...
class ProductApiService(Component):
    ...
    def update_stockpicking(self, stock_picking_input):
        stockpicking = self.env["stock.picking"].search([('name', '=',stock_picking_input.name)],limit=1)
        update_params = {
            "id": stockpicking.id,
            "state": stock_picking_input.state
        }
        res_stockpicking = self.env["stock.picking"].browse(stockpicking.id)
        res_stockpicking.write(update_params)
        stockPickingInfo = self.env.datamodels["stock.picking.info"]
        res = stockPickingInfo(partial=True)
        res.name = res_stockpicking.name
        res.picking_type_id = res_stockpicking.picking_type_id.name
        res.origin = res_stockpicking.origin
        res.l10n_pe_vat_code = res_stockpicking.partner_id.l10n_latam_identification_type_id.l10n_pe_vat_code
        res.vat = res_stockpicking.partner_id.vat
        return res
